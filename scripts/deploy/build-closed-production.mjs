...
assert.ok(['--closed','--book-milestones'].includes(process.argv[2]));
const milestones=process.argv[2]==='--book-milestones';
...
const settings={VITE_SUPABASE_URL:`https://${ref}.supabase.co`,VITE_SUPABASE_ANON_KEY:anon,
 VITE_FAMILY_PRODUCTION_ENABLED:'false',VITE_FAMILY_CONNECTION_TEST:'false',VITE_FAMILY_SUBJECT_CONNECTION_ENABLED:'false',VITE_PUBLIC_TEST_MODE:'false',VITE_BOOK_MILESTONES_ENABLED:String(milestones)};
...
const outDir=path.join(root,milestones?'output/book-milestones-production-stage':'output/closed-production-stage');
...
fs.writeFileSync(path.join(outDir,'vercel.json'),JSON.stringify({framework:null,buildCommand:'',installCommand:'',outputDirectory:'.',headers:[{source:'/(.*)',headers:[{key:'X-Robots-Tag',value:'noindex, nofollow'}]}]}));
...
const evidenceDir=milestones?'output/book-milestones-production-release':'output/closed-production-release';
fs.mkdirSync(evidenceDir,{recursive:true,mode:0o700});fs.writeFileSync(`${evidenceDir}/build.json`,JSON.stringify(report,null,2));
