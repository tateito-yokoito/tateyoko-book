...
const dir=process.env.QA_PREFLIGHT_DIR || 'output/book-milestones-release-preflight',quote=s=>'"'+s.replaceAll('"','""')+'"';
...
for(const label of process.argv.includes('--production-only')?['production']:['test','production']){
 ...
 try{
  assert.ok(!schema.migrations.some(m=>m.version==='202609230001'),'Already applied');
...
await writeFile(`${dir}/web-book-rehearsal.json`,JSON.stringify(report,null,2));console.log(JSON.stringify(report));
