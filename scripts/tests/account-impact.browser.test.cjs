...
(async()=>{
 ...
 try{
  ...
  const row=page.getByRole('button',{name:/確認用アカウント/});
  await row.waitFor();
  assert.match(await row.innerText(),/所有 3件 · 語り 12件 · 語り足し 6本 · 動画 3本/);
  assert.match(await row.innerText(),/ほか1件/);
  assert.match(await row.innerText(),/非表示 1件/);
  ...
  console.log(`PASS browser: list totals/names, detail, hidden inclusion, deduplication, refreshed confirmation, fail-closed error, cancellation, admin disabled, mobile overflow. Screenshots: ${temp}`);
