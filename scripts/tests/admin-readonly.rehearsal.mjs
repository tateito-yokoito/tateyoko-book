// Recreate the saved release catalog locally; apply only the additive Web Book migration. No remote writes.
...
const dir='/private/tmp/tateyoko-production-supporter-review/output/web-book-source-audit',quote=s=>'"'+s.replaceAll('"','""')+'"';
const files=['202609240004_admin_readonly_customer_view.sql'];
...
for(const label of ['production']){
 const schema=JSON.parse(await readFile(`${dir}/${label}-schema.json`)),db=new PGlite({extensions:{pgcrypto}});
 try{
  assert.ok(!schema.migrations.some(m=>m.version==='202609240004'),'Already applied');
...
await writeFile('/private/tmp/tateyoko-admin-readonly-rehearsal.json',JSON.stringify(report,null,2));console.log(JSON.stringify(report));
