# BOOKの二つの節目 — 本番公開済み（2026-09-22）

最新の公開結果・commit・deployment・rollback先は `book-milestones-production-release-20260922.md` を参照。
...
- TEST専用の実収録確認ページで、実認証・実RPCによるはじまりの問いと収録待機まで確認。音声が初期選択、画面遷移のみではマイク・カメラを起動しない。続いてユーザーから「動作は問題なし」と実収録TESTの確認結果を受領。端末別・5分実時間・全操作の個別証跡をエージェントが取得したわけではない。
...
3. ユーザーの実収録TEST確認を受領して本番反映。実iPhone/Safari・Androidの機種別検証、5分実時間・バックグラウンド中断等の網羅的証跡は未取得。
4. 今回の節目追加を含むSupporterからの写真・編集・BOOK確定・注文全工程は未再走。Family公開gateは引き続きOFFで、今回の公開に含めない。
5. 既存2本の未関連動画を持つ利用者の運用案内を確認する。自動再分類しない。
6. 監査履歴・置換済み／未保存ファイルの保管期限と削除は本変更に含めない。

## 公開状態

`VITE_BOOK_MILESTONES_ENABLED` はソース既定OFF、本番artifactでは明示的にON。リモートTEST・本番DBとも `202609220002` を適用済み。Edgeは変更なし。現在公開中のHPのみを保持してフロントを反映。mainへのpushなし。Family公開gate・allowlist・Cも変更しない。

migrationには既存関数定義の厳密な文字列照合があり、driftならtransaction全体を停止する。本番適用前に最新カタログで再リハーサル済み。実適用でも全229関数の定義・migration履歴を照合し、11の保護データ集合が適用前後で不変であることをtransaction内で検証した。

本番HPの `9b6cd1a` を統合し、HPソース・CTA・HTMLとnoindexヘッダーを保持。別HPタスクのそれ以降の未公開案は混ぜていない。統合artifactを作り、差分確認・build・Smoke後に公開した。
