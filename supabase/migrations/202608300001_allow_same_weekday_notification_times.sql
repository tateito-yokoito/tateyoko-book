...
-- Allow multiple delivery times on the same weekday. Only the exact same
-- weekday and time is considered a duplicate.
alter table public.notification_schedules
  drop constraint if exists notification_schedules_user_weekday_unique;

alter table public.notification_schedules
  drop constraint if exists notification_schedules_user_slot_unique;

alter table public.notification_schedules
  add constraint notification_schedules_user_slot_unique
  unique (user_id, weekday, hour, minute);
...
begin
  ...
  on conflict on constraint notification_schedules_user_slot_unique do update set
